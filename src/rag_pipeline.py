...
class RAGPipeline:
    ...
    def retrieve(self, question: str, top_k=None):
        ...
        return self.retriever.retrieve_context(query=question, top_k=validate_top_k(top_k))
    ...
    def generate_answer(self, question: str, context: str, conversation_history=None):
        ...
        try:
            answer = self.llm.ask(question=question, context=context, conversation_history=conversation_history)
    ...
    def ask(self, question: str, top_k=None, conversation_history=None):
        ...
        answer = self.generate_answer(question=question, context=context, conversation_history=conversation_history)
        return {"question": question, "answer": answer, "context": context, "documents": retrieval_result.get("documents", []), "document_count": retrieval_result.get("document_count", 0)}

    def health_check(self):
        return {"status": "healthy", "retriever": self.retriever.get_retrieval_info(), "llm": self.llm.get_model_info()}

    def get_pipeline_info(self):
        return {"pipeline": "Retrieval Augmented Generation", "version": "1.1", "retriever": type(self.retriever).__name__, "llm": type(self.llm).__name__}
